...
func do_fire(camera, muzzle, spray_amount, attack=ATTACK):
	...
	var rand_x = randf_range(-1, 1) * spray_amount
	var rand_y = randf_range(-1, 1) * spray_amount
	var spray_dir = cam_forward + camera.global_transform.basis.x * rand_x \
								+ camera.global_transform.basis.y * rand_y
